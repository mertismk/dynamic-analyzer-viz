...
Sinks:
  - Name: system
    Args: [0]"""
        ...
        # ИСПРАВЛЕНО: совершенно новая упрощённая функция подсветки
        c_lines = code_str.splitlines()
        line_count = len(c_lines)
        # Простой расчёт: общая высота / количество строк
        line_height = code_block_c.height / line_count


        def highlight_c(line_num, color):
            """Подсвечивает одну строку по номеру (1-based)"""
            rect = RoundedRectangle(
                corner_radius=0.04,
                width=code_block_c.width * 0.88,
                height=line_height * 0.85,  # Чуть меньше чем полная высота строки
                color=color,
                fill_color=color,
                fill_opacity=0.22,
                stroke_width=3,
            )
            # Выравниваем по левому краю блока кода
            rect.align_to(code_block_c, LEFT).shift(RIGHT * 0.15)
            # Позиция Y: от верха блока вниз на (line_num - 0.5) строк
            y_pos = code_block_c.get_top()[1] - line_height * (line_num - 0.5)
            rect.move_to([rect.get_center()[0], y_pos, 0])
            return rect


        # Подсветки в YAML блоке
