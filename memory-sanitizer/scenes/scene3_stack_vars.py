...
class SuccessfulCase(Scene):
    def construct(self):
        title = Text("MSan: Успешная проверка", font_size=36, color=GREEN)
    ...
    delete[] ptr;
        ...
        code_label = Text("Корректный код", font_size=16, color=GREEN)
        ...
        line_height = code.height / 19

        phase1 = Text("Этап 1: Маркировка памяти", font_size=16, color=YELLOW)
        phase1.next_to(title, DOWN, buff=0.25)

        highlight1 = RoundedRectangle(
            corner_radius=0.05,
            width=code.width * 0.88,
            height=line_height * 2.14,
            color=RED,
            fill_color=RED,
            fill_opacity=0.25,
            stroke_width=0,
        ...
        highlight1.move_to(
            code.get_top() + DOWN * (line_height * 4.00), aligned_edge=UP
        )
        ...
        self.play(*[FadeIn(v) for v in shadow_vals], run_time=0.6)
        self.wait(1)
        self.play(FadeOut(highlight1), FadeOut(phase1))

        phase2 = Text(
            "Этап 2: Инициализация всех элементов", font_size=16, color=YELLOW
        )
        ...
        highlight2.move_to(
            code.get_top() + DOWN * (line_height * 8.05), aligned_edge=UP
        )

        self.play(Write(phase2), FadeIn(highlight2))

        heap_vals = VGroup()
        shadow_clean_vals = VGroup()

        for i in range(10):
            val_i = Text(str(i * 2), font_size=10, color=GREEN, weight=BOLD).move_to(
                heap_cells[i]
            )
            ...
            heap_vals.add(val_i)
            shadow_clean_vals.add(shadow_clean)

            self.play(
                FadeIn(val_i, scale=1.2),
                ReplacementTransform(shadow_vals[i], shadow_clean),
                Flash(heap_cells[i], color=GREEN, flash_radius=0.2),
                run_time=0.25,
            )
        ...
        phase3 = Text("Этап 3: Проверка — Успех!", font_size=16, color=GREEN)
        ...
        highlight3.move_to(
            code.get_top() + DOWN * (line_height * 12.8), aligned_edge=UP
        )

        self.play(Write(phase3), FadeIn(highlight3))

        check_frame = SurroundingRectangle(
            shadow_cells[0], color=GREEN, buff=0.08, stroke_width=4
        )

        self.play(Create(check_frame))
        self.play(Indicate(shadow_clean_vals[0], color=GREEN, scale_factor=1.4))
        ...
        success_box.next_to(shadow_cells[0], RIGHT, buff=0.35)

        success_text = VGroup(
            Text("✓ SUCCESS!", font_size=13, color=GREEN, weight=BOLD),
            Text("Все данные инициализированы", font_size=10, color=WHITE),
        ).arrange(DOWN, buff=0.1)
        success_text.move_to(success_box)

        self.play(FadeIn(success_box), Write(success_text))
        self.wait(2)
